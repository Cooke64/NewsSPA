...
export default function GoodItem({item}) {
    const checkinCart = () => {
        var products = JSON.parse(localStorage.getItem('items'))[0]
        console.log(products[0])
        if (! products){
            return false
        }
        if (products && item.name in products.name){
            return true
        }
        return false
    }
    const resCheckCart = checkinCart()
    const [incart, setInCart] = React.useState(resCheckCart)
