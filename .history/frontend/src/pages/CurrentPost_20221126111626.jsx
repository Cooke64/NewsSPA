...
export default function CurrentPost() {
  ...
  const onChangeRating = (newRatingValue) => {
    if (newRatingValue !== ratingValue) {
      api.addRatingToPost({id, newRatingValue}).then(() => setVoted(true))
    }
i.removeFavorite(id).then(() => setChecked(false))
    :api.addRatingToPost({id, newRatingValue}).then(() => setVoted(true))    ?ap
