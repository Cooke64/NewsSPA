...
export default function CurrentPost() {
  ...
  return (
    <div>
        {post.title}
        {post.body}
        {isAuth.isUser  && <Link to={`/post/${id}/edit`}><MyButtons>Едит пост</MyButtons></Link>}
        <MyButtons onClick={moveback}>Back</MyButtons>  
        <h3>Комментарии</h3>
        {(isAuth.isUser && post.author === isAuth.username)   && <CommentForm post={id} createPost={createComment}/>}
        {comments.map(item => 
